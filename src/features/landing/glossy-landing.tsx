...
export function GlossyLanding({ userName = "" }: { userName?: string }) {
  ...
  const latestAuditedCourse = hasHydrated
    ? Object.values(auditedCourses).sort(
        (left, right) => Date.parse(right.auditedAt) - Date.parse(left.auditedAt)
      )[0]
    : undefined;
          ...
          <div className="relative flex items-center gap-3">
            ...
            {userName ? (
              <Button asChild className="shrink-0 px-3 text-sm sm:px-4">
                <Link href={startStudyingHref}>
                  {latestAuditedCourse ? "Resume studying" : "Start studying"}
